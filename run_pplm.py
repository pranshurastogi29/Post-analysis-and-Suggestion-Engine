...
def set_generic_model_params(discrim_weights, discrim_meta):
    ...
    DISCRIMINATOR_MODELS_PARAMS['generic']['path'] = discrim_weights
...
def run_pplm_example(
    ...
    for i, pert_gen_tok_text in enumerate(pert_gen_tok_texts):
        ...
        generated_texts.append((tokenized_cond_text, pert_gen_text, unpert_gen_text))

    return generated_texts

def to_gpu(model , index: int = 0) -> None:
    """Moves the model to the specified GPU."""

    assert torch.cuda.is_available(), "CUDA is not installed."
    model = model.half()
    model.to(torch.device("cuda", index))
    return model

def load_language_model(model_path , device='cpu'):
...
def generate(
        ...
        while True:
            ...
            if lstrip:
              ...
              gen_texts = [','.join(text.split('<|endoftext|>')) for text in gen_texts]
            ...
            if not return_as_list:
              if prompt:
                ...
                gen_texts = [text.replace(prompt_text, f"\033[1m{prompt_text}\033[0m", 1)
